...
|HOSTED_MANAGEMENT_CLUSTER| The OpenShift CI hosted management cluster used to provision the ephemeral cluster| "hosted-mgmt2"| false|
|HYPERSHIFT_NODE_COUNT| Number of worker nodes to provision in the ephemeral HyperShift cluster| "3"| false|
...
#### HOSTED_MANAGEMENT_CLUSTER (Optional)

The OpenShift CI hosted management cluster used to provision the ephemeral cluster. Defaults to
`hosted-mgmt2`.

#### HYPERSHIFT_NODE_COUNT (Optional)

Number of worker nodes to provision in the ephemeral HyperShift cluster. Defaults to `3`.
