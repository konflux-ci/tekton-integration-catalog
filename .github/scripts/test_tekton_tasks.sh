...
# - tkn, kubectl, yq, jq installed
#
# Examples of usage:
# export TEST_ITEMS="tasks/linters/hadolint/0.1 some/other/dir"
# ./test_tekton_tasks.sh
#
# or
#
# ./test_tekton_tasks.sh tasks/linters/hadolint/0.1 some/other/dir

# Validate required dependencies
for cmd in tkn kubectl yq jq; do
...
Example: ./.github/scripts/test_tekton_tasks.sh tasks/linters/hadolint/0.1
...
for ITEM in "${TEST_ITEMS[@]}"; do
  ...
  # Extract the task directory (everything up to and including version)
  # For paths like tasks/linters/hadolint/0.1 or tasks/linters/hadolint/0.1/tests/test-foo.yaml
  # we want tasks/linters/hadolint/0.1
  if [[ "$ITEM" == *tests/test-*.yaml ]]; then
    # If it's a test yaml, extract the task dir by removing /tests/test-*.yaml
    # shellcheck disable=SC2001  # sed is appropriate here for regex pattern matching
    TASK_DIR=$(echo "$ITEM" | sed 's|/tests/test-.*\.yaml$||')
  else
    # It's already a task directory
    TASK_DIR="$ITEM"
  fi

  # Extract version (last path component)
  TASK_VERSION=$(basename "$TASK_DIR")

  # Find the task yaml file in the task directory
  # The yaml file is named after the task (last non-version component)
  TASK_YAML_FILES=("$TASK_DIR"/*.yaml)
  if [ ${#TASK_YAML_FILES[@]} -eq 0 ]; then
    echo "ERROR: No task yaml file found in $TASK_DIR"
    exit 1
  fi

  # Get the task name from the yaml filename (without .yaml extension)
  TASK_PATH="${TASK_YAML_FILES[0]}"
  ...
  for TEST_PATH in "${TEST_PATHS[@]}"; do
    ...
    # Wait up to 30 seconds for pipeline to be ready (5s * 6 iterations)
    while ! ${KUBECTL_CMD} -n "${TEST_NS}" get pipeline "$TEST_NAME" > /dev/null 2>&1; do
      echo "DEBUG: Pipeline $TEST_NAME not ready. Waiting 5s..."
      sleep 5
    done

    PIPELINERUN=$(tkn p start "$TEST_NAME" -n "${TEST_NS}" -o json | jq -r '.metadata.name')
    echo "INFO: Started pipelinerun: $PIPELINERUN"
    # Allow pipelinerun object to appear with status condition (usually < 1s)
    sleep 1
    # Poll status until pipelinerun completes (status changes from Unknown)
    ...
    if [ "$ASSERT_TASK_FAILURE" != "null" ]; then
      if [ "$PR_STATUS" == "True" ]; then
        ...
        TASKRUN_STATUS=$(${KUBECTL_CMD} get tr "$TASKRUN" -n "${TEST_NS}" -o=jsonpath='{.status.conditions[0].status}')
        if [ "$TASKRUN_STATUS" != "False" ]; then
