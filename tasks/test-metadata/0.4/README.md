...
### Instrumented Container Image Results (v0.4)

18. **instrumented-container-image**: The container image reference containing the instrumented code.
    - **Description**: This result contains the full image reference to the instrumented container image. The instrumented image is detected by examining the cosign attestation data for build tasks (tasks with "build" in their name) that have the build argument "ENABLE_COVERAGE=true". If no such task is found, this will be an empty string. When a digest is present, the tag is automatically removed from the image reference (e.g., `repo:tag@sha256:...` becomes `repo@sha256:...`) since the digest provides an immutable reference.

19. **instrumented-container-repo**: The container repository extracted from the instrumented container image.
    - **Description**: This result contains the repository part of the instrumented container image (e.g., quay.io/org/repo). If no instrumented image is found, this will be an empty string.

20. **instrumented-container-tag**: The container tag extracted from the instrumented container image.
    - **Description**: This result contains the tag part of the instrumented container image. If no tag is present in the instrumented image, it defaults based on the event type: "on-pr-{revision}" for pull requests or "{revision}" for push events. If no instrumented image is found, this will be an empty string.
...
### Instrumented Container Image Detection (v0.4)
- **Code Coverage Support**: Automatically detects instrumented images built with coverage enabled
- **Build Attestation Parsing**: Analyzes cosign attestation data to find build tasks with ENABLE_COVERAGE=true
- **Image Reference Extraction**: Extracts IMAGE_REF from build tasks that produce instrumented images
- **Graceful Handling**: Returns empty strings when no instrumented image is found
- **Multiple Format Support**: Handles images with tags, digests, or both
- **Tag Normalization**: Automatically removes tags when digests are present to provide clean, immutable references

Example output:
```
[INFO] Cachi2 artifact: found (sha256:abc123...)
[INFO] Source artifact: not found
[INFO] No tag found, defaulting to: on-pr-abc123def
[INFO] Instrumented image: found (quay.io/example/app@sha256:xyz789...)
```

Or when no instrumented image is built:
```
[INFO] Cachi2 artifact: not found
[INFO] Source artifact: found (sha256:def456...)
[INFO] Instrumented image: not found
```
